...
func TestGetSeedListUpdater(t *testing.T) {
	t.Parallel()
	...
	exporters := []*Exporter{
		New(&Opts{
			...
			OriginURI: "mongodb+srv://server.example.com",
			IsSRV:     true,
		}),
		New(&Opts{
			URI:       "mongodb+srv://localhost",
			OriginURI: "mongodb+srv://localhost?db=test",
			IsSRV:     true,
		}),
	}
	...
	go SeedListUpdater(ctx, exporters, 1, log)
	time.Sleep(2 * time.Second)
	opts1 := exporters[0].GetOpts()
	assert.Equal(t, "mongodb://mongo1.example.com:17001,mongo2.example.com:17002,mongo3.example.com:17003/?authSource=admin", opts1.URI)
	opts2 := exporters[1].GetOpts()
	assert.Equal(t, "mongodb+srv://localhost?db=test", opts2.URI)
	ctx.Done()
}
