...
func SeedListUpdater(ctx context.Context, exporters []*Exporter, updateInterval int, log *logrus.Logger) {
	for {
		select {
		case <-ctx.Done():
			return
		case <-time.After(time.Duration(updateInterval) * time.Second):
			log.Debug("Updating seedlists from SRV")
			for _, e := range exporters {
				opts := e.GetOpts()
				if opts.IsSRV {
					e.SetURI(GetSeedListFromSRV(e.opts.OriginURI, log))
				}
			}
		}
	}
